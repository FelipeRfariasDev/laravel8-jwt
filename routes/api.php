...
Route::group([ 'middleware' => 'auth-jwt', 'prefix' => 'auth' ], function () {
    ...
    Route::get('comentarios/{post_id}', [ComentarioController::class, 'show'])->name('auth.comentarios.show');
    Route::post('comentarios', [ComentarioController::class, 'store'])->name('auth.comentarios.store');
    Route::delete('comentarios/{id}', [ComentarioController::class, 'destroy'])->name('auth.comentarios.destroy');
